...
if(request.getParameter("SupportNo") == null) {
    response.sendRedirect(request.getContextPath()+"/SupportList.jsp");
} else {
    int SupportNo = Integer.parseInt(request.getParameter("SupportNo"));
    System.out.println("boardModify param boardNo:"+SupportNo);
...
<div id="wrap">
    <div class="board_wrap">
        <div class="board_title">
            <strong>지원서 수정</strong>
        </div>
        <form action="<%=request.getContextPath()%>/controller/SupportModify.jsp?no=<%=support.getNo()%>" method="post" enctype="multipart/form-data">
            <div class="board_write_wrap">
                <div class="board_write">
                    <div class="title">
                        <dl>
                            <dt>지원서 번호</dt>
                            <dd><input name="SupportNo" value="<%=support.getNo()%>" type="text" readonly="readonly"/></dd>
                        </dl>
                        <dl>
                            <dt>제목</dt>
                            <dd><input name="title" value="<%=support.getTitle()%>" id="title" type="text"/></dd>
                        </dl>
                        <dl>
                            <dt>비밀번호 확인</dt>
                            <dd><input style="width:50%; padding:5px; margin:10px;" name="passwd" id="title" type="password"/></dd>
                        </dl>
                        <dl>
                            <dt>기존 파일명</dt>
                            <dd><%=support.getFilename()%></dd>
                            <dd><input name="oldfilename" value="<%=support.getTitle()%>" id="title" type="text" readonly hidden/></dd>
                        </dl>
                        <dl>
                            <dt>새 파일</dt>
                    ...
                    <div class="cont">
                        <textarea name="content" id="content" rows="5" cols="50"><%=support.getContent()%></textarea>
                    </div>
                </div>
                <div class="bt_wrap">
                    <input class="reviewbutton" type="submit" value="글수정"/>
